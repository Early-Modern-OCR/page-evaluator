...
abstract class HOCRToken(id: String, text: String) extends OCRToken(id, text) with SpellChecker with TextTransformer {
   ...
   */
  lazy val correctTransformationsOfCleanedToken = transformations(cleanedText).filter(t => isCorrect(t.text))

  lazy val correctTransformationsOfOriginalToken =
    // if transformation rules exist for the punctuation at the beginning and end of the token, include the
    // suggestions generated by applying those transformations to the unclean token
    if (text.takeWhile(!_.isLetterOrDigit).forall(punct => transformRules.contains(punct.toString)) &&
      text.reverse.takeWhile(!_.isLetterOrDigit).forall(punct => transformRules.contains(punct.toString)))
      transformations(text).filter(t => isCorrect(t.text))
    else
      Nil

  lazy val correctTransformations = correctTransformationsOfCleanedToken ++ correctTransformationsOfOriginalToken
  ...
  lazy val heuristicReplacements = {
    ...
    if (noPunctText.length != cleanedText.length && noPunctText.length > HEUR_MIN_NO_PUNCT_LEN) {
